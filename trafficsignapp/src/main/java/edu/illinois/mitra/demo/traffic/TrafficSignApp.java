...
public class TrafficSignApp extends LogicThread {
	...
	private String name = gvh.id.getName();
	private int id = gvh.id.getIdNumber();
	...
	public TrafficSignApp(GlobalVarHolder gvh) {
		...
		String letter = Character.toString((char)(65 + robotIndex ));
		...
		for(ItemPosition i : gvh.gps.getWaypointPositions()){
			if(i.name.startsWith(letter) ){
				destinations.add(new ItemPosition(i));
			}
		}
