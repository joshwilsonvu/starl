 ...
 *
 * Similar to ArrowTravelApp, not working, wouldn't use.
 * @author Lucas Buccafusca
 * @date 05-31-2012
 */

public class FlockingWithoutComms extends LogicThread {
    ...
    public FlockingWithoutComms(GlobalVarHolder gvh) {
        ...
        gvh.comms.addMsgListener( this,98);
        gvh.comms.addMsgListener( this,97);
        gvh.comms.addMsgListener( this,96);

        // Make sure waypoints were provided
        if (gvh.gps.getWaypointPositions().getNumPositions() == 0)
            System.out.println("This application requires waypoints to travel to!");
                ...
                case BROADCAST:
                    ...
                    if (!iamleader) {
                        MessageContents rm = new MessageContents(
                                gvh.gps.getMyPosition().getX() + " "
                                        + gvh.gps.getMyPosition().getY() + " " + robotNum);
    ...
    protected void receive(RobotMessage m) {
        ...
        if (m.getMID() == 98) {
            if (!toVisit.isEmpty()) {
                ...
                double distance_y = gvh.gps.getPosition(leader).getY() - y_coordinate;
                ...
                if (y_coordinate > gvh.gps.getMyPosition().getY()) {
                    synchronized (stage) {
                        stage = STAGE.WAIT;
                    }
                }
...
*/
            ItemPosition go_to = new ItemPosition("goHere",Integer.parseInt(data[1]),Integer.parseInt(data[2]),0);
            gvh.plat.moat.goTo(go_to);
